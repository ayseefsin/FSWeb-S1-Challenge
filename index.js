...
function profilListesi(fenomenler) {
  let fenomenlerinIsimleri = [];
  for (let i = 0; i < fenomenler.length; i++) {
    fenomenlerinIsimleri.push(fenomenler[i].profile);
  }
  return fenomenlerinIsimleri;
}
...
function fenomenSil(fenomenler, iKey) {
  let fenomenlerYeni = [];
  for (let i = 0; i < fenomenler.length; i++) {
    if (i != iKey) {
      fenomenlerYeni.push(fenomenler[i]);
    }
  }
  return fenomenlerYeni;
}
...
function fenomenEkle(fenomenler, sira, isim, takipçi, paylaşim, site) {
  fenomenler[fenomenler.length] = {
    number: sira,
    profile: isim,
    followers: takipçi,
    posts: paylaşim,
...
function enFenomenler(fenomenler) {
  ...
  for (let i = 0; i < fenomenler.length; i++) {
    if (fenomenler[i].followers > 100000000) {
      stars.push(fenomenler[i].profile);
    }
  }
  return stars;
}
...
function fenomenGonderimSayisi(fenomenler, fenomenismi) {
  ...
  for (let i = 0; i < fenomenler.length; i++) {
    if ((fenomenler[i].profile = fenomenismi)) {
      paylasimSayisi = fenomenler[i].posts;
    }
  }
...
function platformaGoreCokGonderiYapanFenomen(fenomenler, uygulama) {
  let uygulamaStars = fenomenler;
  let uygulamaStarsPaylasimSayisi = 0;
  for (let i = 0; i < uygulamaStars.length; i++) {
    if (
      typeof uygulamaStars[i].posts === typeof uygulamaStarsPaylasimSayisi &&
      uygulamaStars[i].platform === uygulama &&
      uygulamaStars[i].posts > uygulamaStarsPaylasimSayisi
    ) {
      uygulamaStarsPaylasimSayisi = uygulamaStars[i].posts;
    }
  }
  for (let i = 0; i < uygulamaStars.length; i++) {
    if (
      ...
      return uygulamaStars[i].profile;
  }
}
